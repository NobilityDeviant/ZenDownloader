...
class CoreChild {
    ...
    fun shutdown(update: Boolean = false) {
        if (update) {
            ...
            stop()
            GlobalScope.launch { killAllDrivers(true) }
            return
        }
        if (isRunning) {
    ...
    //shutdown code 100 triggers the new bootstraps update function
    private suspend fun killAllDrivers(
        update: Boolean = false
    ): Nothing = withContext(Dispatchers.Default) {
        ...
        if (drivers.isEmpty()) {
            ...
            exitProcess(if (update) AppInfo.UPDATE_CODE else 0)
        }
