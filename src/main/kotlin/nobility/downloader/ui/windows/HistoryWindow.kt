...
class HistoryWindow {
                            ...
                            defaultButton(
                                ...
                                DialogHelper.showConfirm(
                                    ...
                                    if (historyData.isEmpty()) {
                                        showToast("There's no history to clear.")
                                        return@showConfirm
                                    }
                                    BoxHelper.shared.historyBox.removeAll()
                                    historyData.clear()
                                    showToast("History successfully cleared.")
                                }
                            }
                            defaultButton(
                                ...
                                if (historyData.isEmpty()) {
                                    showToast("There's no history to check.")
                                    return@defaultButton
                                }
                                scope.launch {
                                    showToast("Checking ${historyData.size} series for new episodes...")
    ...
    private suspend fun checkAllHistoryForNewEpisodes(): Resource<CheckedResults> = withContext(Dispatchers.IO) {
        ...
        clearHistoryEnabled.value = true
        checkForEpisodesButtonEnabled.value = true
        Resource.Success(CheckedResults(
            seriesUpdated,
            episodesUpdated)
        )
    }
